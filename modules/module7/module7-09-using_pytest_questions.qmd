...
<script>
    generateQuiz(
        ...
        {
        'In the same Jupyter notebook': 'This would defy the DRY principle if we wanted to test our function in multiple different notebooks.',
        'In the same file as the function you are testing': 'Not if we want to use <code>pytest</code>!',
        'In a separate <code>.py</code> file': '',
        'In a separate Jupyter notebook': '<code>pytest</code> would not be able to check the tests in a separate Jupyter notebook.',
        },
        'In a separate <code>.py</code> file',
    );
</script>
