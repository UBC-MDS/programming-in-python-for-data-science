...
We do this using a scatter plot which uses `.mark_circle()`. We can assign `sugars` as the `x` variable and `ratings` as the `y` variable from the `cereal` dataframe we have been using. 
