...
The question now is "Is there must be a better way?" and luckily there is!
...
The `return` statement is crucial to a function for returning a desired output.  Here, our function returns a new list.  If the function does not return anything then you won't get a value to assign to an object.
...
You could get even more fancy, and put all the list of numbers into a list (so you have a list of lists).
...
There is some ambiguity for how and when to design a function.  For instance: 
...
Mike Gelbart, co-creator of this course may prefer something like this instead:
...
We then can use a loop or comprehension to apply `squares_a_number` to every element of a list.
...
This is his, preference though and not a set rule. 
Notes: Script here
<html>
<audio controls >
  <source src="/placeholder_audio.mp3" />
