...
If we wanted to filter all the rows that contain null values and not just in the `Distance` column, we can use the verb `.any()` on the full dataframe.
...
Alternatively if you have a column missing a large portion of data, the best option maybe to drop that column instead of the rows with missing values.
