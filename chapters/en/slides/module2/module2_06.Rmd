...
This is only the tip of the iceberg.  There are many others that are helpful when reading in our data, such as `index_col`, `header`, `nrows`, and `usecols`. 
...
Notes: 
`index_col` is an argument that indicates which column will be acting as the index label. In most of the cases we have encountered, we did not use this argument and instead relied on the pandas default, which is to use ascending integers for the index.   

We can, however, specify a column in the data to become the index. It's in our best interest that the column we choose have unique values.   
For our `cereal.csv` let's specify the `name` column as our index. 

The `index_col` argument also take in positions. The `name` column in our data is in the 0th position so we can also specify the index like we show here with `index_col=0`. 
...
We use `header=2` to indicate that the data actually begins at position 2.
...
`nrows` is an argument that is useful when you only want to load in part of the dataframe. 

Perhaps the file you have is large and you only want a sample of it.  `nrows` will limit the number of rows that you read in.
