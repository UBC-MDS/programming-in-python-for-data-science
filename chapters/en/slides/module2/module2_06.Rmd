...
We can, however, specify a column with to be the index. It's in our best interest that the column we choose have unique values.   
...
The `index_col` argument also take in positions. the `name` column in our data in in the 0th position so we can also specify the index like so:
