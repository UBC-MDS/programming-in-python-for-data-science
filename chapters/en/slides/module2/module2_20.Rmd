...
When we use the `.assign()` verb, it creates a new dataframe with only the rows that meet the condition ```cereal['mfr'] == 'Q'``` .  
...
Filtering without `.loc[]` is a bit more readable and more efficient when doing analysis. 
...
It gives us an opportunity to name the new dataframe something different and keep our original dataframe untransformed. 

This prevents the need to re-read in our data to get our original dataframe back (which could take a long time if the dataset is large).
