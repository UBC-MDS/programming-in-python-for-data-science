...
a `NoneType` is quite different than `NaN`. `NaN` is used for missing numerical values and is classified as type `float` but a `None` value is an actual data type. 
...
We have seen this data type when we were filtering our dataframes with conditions.  A condition is evaluated and produces a column stating whether the condition was met or not. 
