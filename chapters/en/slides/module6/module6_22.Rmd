...
Being inflexible can cause you end up writing more functions and/or violating the DRY principle. 
...
Although this may seem nice, we may want to break this up into multiple smaller functions. For example, what if we don't want the plot? Perhaps the plots is just something we wanted a single time and now we are committed to it for each time we use the function. 
