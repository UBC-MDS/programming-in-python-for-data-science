...
It works somewhat well but what happens if we try to use it with a input string instead of a list.
...
Another good thing to check for if all the elements in the list are not negative numbers.

We can use a `for` loop to check that each element is greater or equal to 0. 

This would avoid issues like taking the square root of negative one, if the list contained negative values and the exponent was one half.
However, this might be too strict of a limitation in a real function because we might still want to do things like -5 squared.
