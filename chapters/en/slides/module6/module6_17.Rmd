...
We'll be implementing unit tests in Python using `assert` statements." `assert` statements are just one way of implementing this. 
...
Instead, programmers often use a different approach: writing tests using `assert` statements before the actual function.  This is called Test-Driven Development (TDD).
...
Often we have an idea of what our function should be able to do, and what the function operation output is expected.
...
Let's write another `assert` for a differently sized list as well as different values for both input arguments `numerical_list` and `exponent`. 
...
Here we see that 0 cannot be raised to a negative integer, so our function isn't perfect!
...
In this case, we would need to correct the code in our function to handle this weird case or inform the user using `Exceptions` that our function cannot accept lists containing 0 if the exponent is negative. 
...
The function below is something we might have envisioned.
...
The values we chose in our columns need to be simple enough to calculate the expected output of our function. 

Just like how we used made tests using calculations we know to be true, we do the same using a simple dataset we call **helper data**. 
...
Using our `exponent_a_list()` function as an example, we include the function's initialization line and the return statement. 
...
As you work on the code, more and more tests of the tests that you wrote will pass until finally all your `assert` statements no longer produce any messages.  
