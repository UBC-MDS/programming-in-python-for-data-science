...
When we learned functions in Module 5, arguments are something we
...
Since exponent takes the second argument position omitting the
`exponent=` part will result in the same output.
...
arguments but we must be careful with the order arguments are defined
and called.

When we defined our arguments in a function, all arguments with default
values need to be defined ***after*** required arguments.
