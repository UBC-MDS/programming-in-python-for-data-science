...
Exceptions disrupt the regular execution. When we raise an exception we
are forcing our own error with our own message if the condition is met.
...
The `Exception` in our previous examples is a more generic message which
is defined by the person who writes code of the function. This isn’t all
incompassing as there are other exceptions that are likely better suited
for the raises we made.
