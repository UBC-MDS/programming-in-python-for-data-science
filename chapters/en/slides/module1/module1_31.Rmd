...
Let say we are interested in the `manufacturer` column. It would be great to express the frequency of each expressed as a bar chart. 
...
Like any other packages we have seen so far, Altair needs to be imported before we can use it.
...
2. We can pass the dataframe we'd like to plot, in this case it's `cereal`:
```python
alt.chart(cereal)
```

3. But what kind of plot do we want?! As we said before, a bar chart in this case would work nicely so lets add the verb `.mark_bar()` to our code. 
```python
alt.chart(cereal).mark_bar()...
```
...
In general, we use `count()` if we are interested in counting the frequency of each of elements within the `x` variable. This gives us all the code necessary for our desired plot:
```python
...
This can be done by specifying the `height` and `width` argument in the `alt.Chart(width=..., height=...)...` function. 
...
Opacity is set with the argument `opacity` in the `mark_circle(opacity=...)` function and accepts values between 0 and 1, with 1 being full intensity.
...
Well that is okay, we can change the colour using the `color` argument in the `.mark_circle(color=...)` function. 
...
Good news though, we can make them bigger to enlarge them, the argument `size` in the `mark_circle(size=...)` should do the trick.
...
To set the `title`, we use the `title` argument to the `.properties(title=..)` function.
