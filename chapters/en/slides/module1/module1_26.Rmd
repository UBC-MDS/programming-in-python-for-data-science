...
We usually like to learn from it. We want to find out about some summary statistics about the data. 
...
On the left-hand side we see a new column. This column contains the names of the different summary statistics that `.describes()` gives us back for our dataset. Let's talk about them: 
...
We can make changes to either limit how much is shown or include more statistics in the dataframe with the additional argument `include="all"` in the `describe` brackets.
...
Here are some examples where we first calculate the mean of the ratings, the sum of the ratings and the median of the ratings. 
