...
Let's start by importing pandas and loading in a dataset named `cereal.csv` and saving it as `cereal`.
...
This doesn't always have to be the case. We can assign columns as the index, but we will discuss this in the next section. 
...
We put in the interval of our desired rows first, and then columns and separate them with a comma.  
...
Or we can do something similar for the end of a dataframe. Let's say now we want the first 6 rows and only the columns from `sugars` onward. 
