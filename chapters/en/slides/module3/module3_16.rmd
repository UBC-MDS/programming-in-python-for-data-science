...
We discussed concatenation in the last section and it how it glues dataframe together, but what if we needed to combine dataframe of different sizes where rows need to be matched up?       
...
<center><img src='/module3/mergex.gif' width="600"></center>
...
With  `.merge()`, we need to identify a column in each dataframe that acts as a connection between them.  It's a column where the values in Dataframe A are the same variable to those in Dataframe B. In the easiest situations both columns are named the same thing, but that doesn't have to be the case.
...
This dataframe has new columns androws not in the `candy` dataframe (`3 Musketeers`, `Aero`, and `Kinder Bueno`).
...
`.merge()` needs arguments that identify a common **key** column.  This is a column present in both dataframes which contain common values.  Key columns can be an index, a regular column, or a combination of both.     
...
We mentioned that the default argument value`inner` which  will only the rows with identifying column values that are present in both dataframes:
...
Here we see that `Coffee Crisp` and `Butterfinger` have complete rows.  Rows from the left dataframe that were not present in the right dataframe are `Skor`, `Smarties` and `Twix` and therefore have `NaN` values for columns from the right table.  The opposite occurs for the values `3 Musketeers`, `Aero` and `Kinder Bueno` which are present in the right dataframe and not the left one.  This results in `NaN` for values in the left dataframe columns. 
...
- `left`: This will only output the rows that are in the left dataframe and if they are missing from the right dataframe, `NaN` values will occur.  No candy bars that are only present in the right dataframe will be present.
