...
Notes: Here we can see the same thing happened as before with `.pivot()` but now we've kept the `mfr` column.
...
<center><img src='/module3/problem_table.png' width="100%"></center>
...
We can do this with the `.duplicated()` verb by putting the columns from the `index` and `columns` arguments in a `subset` argument.  We set the argument `keep` to `False` to make sure all the rows are identified and not just the repeated ones. 
...
We use the argument `axis=0` which refers to the rows in the dataframe and `index` which specifies the row index that we want to drop. In this case we are going to drop the row with calories equal to 130, which is index `1`.
