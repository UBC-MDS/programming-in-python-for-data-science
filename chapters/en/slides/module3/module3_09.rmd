...
Similarly to `.pivot()`, `.melt()` can be used in situations where our data may not meet criterion #2: _Each variable is a single column_.
...
Let's take some time to fully understand the arguments this verb uses.  
  ...
  - `value_vars` are columns that exist but that we want to create a single column from with the labels as the values.
  - `var_name` is the name of the new column that will contain the `value_vars` column labels as values.
