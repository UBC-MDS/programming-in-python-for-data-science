...
Unlike `pd.concat()`, where dataframes are joined on common indexes, `.merge()` needs arguments that identifies a common **key** column. This is a column present in both dataframes which contain common values. Key columns can be an index, a regular column or a combination of both. 
For example:  
Dataframe A can have the index label `cereal` and Dataframe B could have a column (that is not the index) labelled `product_name` that both share cereal names. We can join these two dataframes together even though Dataframe B has a different index. 
...
In this example we are using, our left dataframe identifying key column is the index labeled `name` and the right dataframe's identifying column is `chocolate_bar`.
