...
We discussed concatenation in the last section and it how it covers several things so what would we need a second verb for? 
...
`.merge()` give more versatility when assigning columns to act as a connection between the dataframes.  For example, we can use the index on one dataframe and a column label on the other, indexes from both dataframes or no indexes at all to make a connection between the two dataframes. 
...
This dataframe has new columns and three new rows not in the `candy` dataframe (`Kinder Bueno`, `5th Avenue` and `Crunch`).
The column in this dataframe that shares values with the `candy` dataframe is named `chocolate_bar`. 
If we were to use `pd.concat()` to join `candy` and `candy2m` while keeping the id_number as the index in `candy2m` it would be a little challenging, however `.merge()` makes this a little easier for us.  
