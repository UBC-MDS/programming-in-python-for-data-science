...
This is a little different than before where if we wanted a single column from a dataframe we used the notation `df[["column]]`. When we are using the methods above like `df.sum()` we only a single pair of square parentheses for the column object. 
...
Firstly, in a similar mannar to calculating single statistics, we use single parentheses to get the column values instead of double parentheses. 
