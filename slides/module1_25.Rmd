...
Up untill now,  we discussed getting a single column from a dataframe using double parentheses - `df[['column-name']]`. For frequency tables, however, we only use single parentheses to obtain the column values.
...
If we used double parentheses with `pd.value_counts()` we would get an error along the lines of `AttributeError: 'DataFrame' object has no attribute 'value_counts'`. Take care with using the correct number of parentheses. 
