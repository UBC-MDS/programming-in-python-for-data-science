...
We use `header=2` to indicate where our dataframe begins. (2 being the index position or the row that containd the column labels)
...
`nrows` is useful when you only want to load in slice of the dataframe.  Perhaps the file you have is large and you only want a sample of it.  `nrows` will limit the number of rows from the file to read in. 
